...
class YOLO(object):
    _defaults = {
        # "model_path": 'logs/Epoch45-Total_Loss6.5160-Val_Loss9.1404.pth',
        "model_path": 'logs/Epoch37-Total_Loss7.1631-Val_Loss7.8896.pth',
    ...
    def generate(self):
        ...
        self.net.load_state_dict(torch.load(self.model_path, map_location="cuda:0"))
    ...
    def detect_image(self, image):
        ...
        top_xmin, top_ymin, top_xmax, top_ymax = np.expand_dims(top_bboxes[:, 0], -1), \
                                                 np.expand_dims(top_bboxes[:, 1], -1), np.expand_dims(top_bboxes[:, 2], -1), np.expand_dims(top_bboxes[:, 3], -1)
